...
- **Does:** Same idea as `meta_ads_scraper.py` but against the Google Ads Transparency Center. Resolves each target via the site's real autocomplete search (loading `?query=X` in the URL does nothing — you must type into the search box and click a suggestion; see the module docstring), then for each matched advertiser visits up to `--max-ads-per-advertiser` individual ad detail pages and runs **tiered text extraction**:
  1. **`iframe_text`** — some ad cards (native/Discovery-style, a subset of "Format: Image") render inside a live `discover_ads` gadget iframe with real, accessible headline/description text and a real landing-page link. When present, extraction is exact (`confidence: 100`).
  2. **`iframe_link_only`** — large IAB display formats (e.g. 300x600) render inside a sandboxed HTML5 `sadbundle` iframe — usually canvas-drawn with no live text, but the click-wrapper `<a href>` decodes to the exact real landing URL via its `adurl` query param, even with no headline/description available.
  3. **`ocr_fallback`** — everything else (most "Format: Text" and plain "Format: Image" cards) is a flattened PNG snapshot with no text anywhere in the DOM. The snapshot is downloaded and run through Tesseract OCR (confidence-filtered per word, grouped into paragraphs, then split into headline/description via a URL-anchored heuristic or, failing that, a positional fallback). Results below a minimum confident-word count are marked `skipped: true` with a `skip_reason` rather than guessed at.
  4. Video ads are tagged `skipped_video`; ads beyond the per-advertiser cap are tagged `not_attempted`.
  Every ad record carries `extraction_method` and `confidence` so OCR-derived text can be told apart from exact iframe text/links downstream.
- **Advertiser matching requires a confident name match, not just word overlap.** A suggestion is only auto-selected if the searched name appears as a *contiguous substring* of the suggestion's name (tolerating wrapping like "The X Inc"/"X, LLC", but rejecting a different company that merely contains the same words — e.g. "Power Digital" no longer matches "Power Link Digital Group", since "link" breaks the contiguity). When no suggestion clears that bar, the result is `search_method: 'low_confidence_match'` with a `rejected_candidate` field for manual review — `ads_found` stays `false` rather than auto-selecting a guess.
- **Client-campaign filtering (domain-based, primary tier).** Agencies often run Google Ads on behalf of clients through their own MCC account — e.g. Wpromote's matched account showed ads for Gold's Gym and Netskope, not Wpromote's own brand. After extraction, each ad's resolved domain (from a real `landing_url`, then `display_url_text`, then headline/ad_text as fallback) is compared against the target's own `website` column. A mismatch retags the ad as `extraction_method: 'filtered_client_campaign'` (original method preserved in `pre_filter_method`) and excludes it from `landing_page_urls` seeding. `--names` mode looks up `website` from `agency_benchmarks` read-only (no writes) so this filter is testable without DB target rows.
- **Content-based review flag (secondary tier, `needs_review`).** The domain filter above has nothing to check when an ad's OCR'd/iframe text contains no extractable domain at all (e.g. a noisy OCR crop that missed the visible URL) — without this tier it silently fell through as a confirmed `self_brand` ad even when the copy was clearly unrelated client content (real example: Hawke Media ads for "PHOTOGRAPHY AND THE BLACK ARTS MOVEMENT" — more Getty Museum content with no domain in the crop, slipping through next to correctly-filtered `getty.edu` ads from the same advertiser). So when the agency's own `website` is on file *and* no domain resolves for a specific ad, the scraper falls back to checking whether the agency's own name (or a close variant) appears anywhere in that ad's headline/body text. If it doesn't, the ad is retagged `extraction_method: 'needs_review'` (original method preserved in `pre_filter_method`, reason in `filter_reason`) and excluded from both `landing_page_urls` seeding and the `ads_self_brand_count` tally — it is *not* auto-saved as a confirmed self-brand benchmark, just surfaced for manual review. Ads where a domain *did* resolve (and matched the agency's own) skip this check entirely; it only fires in the domain filter's blind spot. Per-advertiser results carry an `ads_needs_review_count` alongside `ads_self_brand_count`/`ads_filtered_client_count` so this bucket is visible without reading every ad.
- **System dependency:** the `tesseract` binary must be installed separately from `requirements.txt` (macOS: `brew install tesseract`) for the OCR fallback to work.
- **Run:** `python3 google_ads_scraper.py --type all --limit 50` (or `--test --names Tinuiti Wpromote` for a dry run against specific advertisers, no DB involved)
- **Flags:** `--type` (choice: `agency`/`authority`/`all`, default `all`), `--limit` (int, default 50), `--test` (flag, dry run), `--names` (one or more advertiser names, implies `--test`), `--headless` (flag), `--max-ads-per-advertiser` (int, default 5)
- **Writes to:** merges into `agency_benchmarks.meta_ads` / `authority_sources.paid_amplification` (adds a `google_ads` key without clobbering existing Meta data); also seeds `landing_page_url_candidate` rows in `raw_source_data` from real `landing_url` values found via the `iframe_text`/`iframe_link_only` paths, excluding anything tagged `filtered_client_campaign` or `needs_review` (the OCR path has no real href otherwise, only a best-effort `display_url_text` read off the image).
...
- Most scrapers accept `--dry-run` or `--test` to print what they'd do without writing to the DB (`reddit_browser_scraper.py`, `run_full_scrape.py`, `magazine_discovery_scraper.py`, `email_intelligence_scraper.py`, `meta_ads_scraper.py`, `google_ads_scraper.py` — the latter also takes `--names` to test specific advertisers without touching `agency_benchmarks`/`authority_sources` at all).
